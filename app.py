...
model = pickle.load(open('RitikBangerNLP.pkl','rb'))   


def review(text):
  dataset = pd.read_csv('NLPdataset11EndSem.csv')
...
html_temp = """
   ...
   <center><p style="font-size:25px;color:white;margin-top:10px;"Machine Learning End Sem Exam</p></center> 
   </div>
   </div>
   </div>
   """
...
text = st.text_area("Input your Tweet")

if st.button("Analyze the Tweet"):
  result=review(text)
  st.success('Model has predicted {}'.format(result))
      
if st.button("About"):
  st.subheader("Developed by Ritik Banger")
  st.subheader("Reg. No: PIET18CS124")
html_temp = """
   <div class="" style="background-color:orange;" >
   <div class="clearfix">           
   <div class="col-md-12">
   <center><p style="font-size:20px;color:white;margin-top:10px;">Machine learning End Sem Exam</p></center> 
   </div>
   </div>
   </div>
   """
